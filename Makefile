...
BIN_NAME=poc-go-kinesis

VERSION := $(shell grep "const Version " version.go | sed -E 's/.*"(.+)"$$/\1/')
GIT_COMMIT=$(shell git rev-parse HEAD)
GIT_DIRTY=$(shell test -n "`git status --porcelain`" && echo "+CHANGES" || true)
IMAGE_NAME := "superliuwr/poc-go-kinesis"

default: test

help:
	@echo 'Management commands for poc-go-kinesis:'
	@echo
	@echo 'Usage:'
...
build-docker:
	@echo "building ${BIN_NAME} ${VERSION}"
	docker build -t poc-go-kinesis:build -f Dockerfile.build .
	docker run --name=poc-go-kinesis -v $(GOPATH):/gopath/  poc-go-kinesis:build
	docker rm -f poc-go-kinesis
